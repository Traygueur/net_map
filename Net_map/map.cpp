...
QString getLocalSubnet() {
    ...
    qDebug() << "❌ Impossible de trouver l'IP ou le masque de la carte Wi-Fi.";
    return "";
}
...
void localInfoInXML() {
    ...
    if (!readFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qDebug() << "❌ Impossible d'ouvrir le fichier XML pour lecture.";
        return;
    }
    ...
    if (!doc.setContent(xmlContent)) {
        qDebug() << "❌ Erreur de parsing du XML.";
        return;
    }
    ...
    if (!existingLocals.isEmpty()) {
        qDebug() << "ℹ️ L'élément <local> existe déjà, on ne modifie rien.";
        return;
    }
    ...
    if (!writeFile.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text)) {
        qDebug() << "❌ Impossible de réécrire le fichier XML.";
        return;
    }
    ...
    qDebug() << "✅ Balise <local> ajoutée avec succès.";
}
